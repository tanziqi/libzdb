...
T MysqlResultSet_new(Connection_T delegator, MYSQL_STMT *stmt, int keep) {
        ...
        R->isStoreResult = Connection_isStoreResult(R->delegator);
...
static bool _seek(T R, unsigned long long rowIndex) {
        assert(R);
        mysql_stmt_data_seek(R->stmt, rowIndex);
        return true;
}
