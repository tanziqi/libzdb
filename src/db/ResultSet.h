...
/**
 * According to the given [row index], move the cursor of the result set to the specified row.
 * The [row index] range is [0 - stmt->rows-1].
 *
 * @Note!!! That it is not row pointer by @mysql_stmt_tell returned 
 * @Note!!! Be sure to call the @Connection_setStoreResult(Con, true) function before using it
 *
 * @param R A ResultSet object
 * @return true if the new current row is valid; false if there are no
 * more rows
 * @exception SQLException If a database access error occurs
 */
bool ResultSet_seek(T R, unsigned long long rowIndex);
...
unsigned int ResultSet_getUInt(T R, int columnIndex);
...
unsigned int ResultSet_getUIntByName(T R, const char *columnName);
